...
class ReadMoreTextState extends State<ReadMoreText> {
  static final RegExp _nonCapturingGroupPattern = RegExp(r'\((?!\?:)');
  ...
  ValueNotifier<bool> get _effectiveIsCollapsed =>
      widget.isCollapsed ?? (_isCollapsed ??= ValueNotifier<bool>(true));
  ...
  @override
  void didUpdateWidget(final ReadMoreText oldWidget) {
    super.didUpdateWidget(oldWidget);

    if (widget.isCollapsed == null && oldWidget.isCollapsed != null) {
      final bool oldValue = oldWidget.isCollapsed!.value;
      (_isCollapsed ??= ValueNotifier<bool>(oldValue)).value = oldValue;
    }
  }
  ...
  Widget _builder(
      final BuildContext context, final bool isCollapsed, final Widget? child) {
    ...
    final TextOverflow overflow = widget.overflow ?? defaultTextStyle.overflow;
    final TextWidthBasis textWidthBasis =
    ...
    final TextStyle defaultLessStyle = widget.lessStyle ??
        effectiveTextStyle.copyWith(color: colorClickableText);
    final TextStyle defaultMoreStyle = widget.moreStyle ??
        effectiveTextStyle.copyWith(color: colorClickableText);
    final TextStyle defaultDelimiterStyle =
      ...
      builder: (final BuildContext context, final BoxConstraints constraints) {
        ...
        if (widget.richData != null) {
          ...
          dataTextSpan = TextSpan(
            ...
            children: <InlineSpan>[widget.richData!],
          );
        ...
        switch (widget.trimMode) {
          case TrimMode.length:
            // Constructed by ReadMoreText.rich(...)
            if (widget.richData != null) {
              final _TextSpanTrimResult trimResult = _trimTextSpan(
            ...
            // Constructed by ReadMoreText(...)
              ...
              if (widget.trimLength < widget.data!.runes.length) {
                final TextSpan effectiveDataTextSpan = isCollapsed
                    ? _trimTextSpan(
                        textSpan: dataTextSpan,
                        spanStartIndex: 0,
                        endIndex: widget.trimLength,
  ...
  TextSpan _buildAnnotatedTextSpan({
    ...
    data.splitMapJoin(
      ...
      onMatch: (final Match regexMatch) {
        final String matchedText = regexMatch.group(0)!;
        ...
        final TextSpan content = matchedAnnotation.spanBuilder(
          text: matchedText,
          textStyle: textStyle,
        );
  ...
  _TextSpanTrimResult _trimTextSpan({
    ...
    if (children != null) {
      for (final InlineSpan child in children) {
        if (child is TextSpan) {
          final _TextSpanTrimResult result = _trimTextSpan(
            textSpan: child,
    ...
    final TextSpan resultTextSpan = didTrim
        ? TextSpan(
            text: textSpan.text,
  ...
  bool _isTextSpan(final InlineSpan span) {
    if (span is! TextSpan) {
      return false;
    }
